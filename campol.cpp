...
std::vector<PolColor> CamPol::transform2camera_screen(std::vector<PolColor>& a)
{
	...
	for(auto ite = a.begin();ite != a.end();++ite){
        ...
        //move point which is back of the camera. near clipping. adhoc clipping
        THR p1 = get_pos_in_camera_coordinate(ite->pol_.p1_);
        THR p2 = get_pos_in_camera_coordinate(ite->pol_.p2_);
        THR p3 = get_pos_in_camera_coordinate(ite->pol_.p3_);
        if(p1.z_ < 0) p1.z_ = 0;
        if(p2.z_ < 0) p2.z_ = 0;
        if(p3.z_ < 0) p3.z_ = 0;

        pc.pol_.p1_ = get_pos_on_screen(p1);
        pc.pol_.p2_ = get_pos_on_screen(p2);
        pc.pol_.p3_ = get_pos_on_screen(p3);
...
sm CamPol::draw(std::vector<PolColor>& d)
{
    int num_pol_draw = 0;
    sm im((int)width_screen_, (int)height_screen_);
    for(auto ite = d.begin();ite != d.end();++ite){
...
sm CamPol::simple_draw(int w, int h, ROT2* r, THR pos, THR dir, std::vector<std::vector<PolColor> >* other_bone)
{
    CamPol cp;
    cp.angle_x_ = 60*M_PI/180;
    cp.angle_y_ = 60*M_PI/180;
