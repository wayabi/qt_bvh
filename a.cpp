...
void sm::sla(int x1, int y1, int x2, int y2, int a, int b, int c)
{
    ...
    while(ax > 1000000) ax /= 100;
    while(ay > 1000000) ay /= 100;
    int y = 0;
    //if(y1 < 0) y = -y1-1;
    //if(y1 > h_) y = -y1+1;
    for(int x=x1;x<=x2;++x){
...
void sm::tri(int x1, int y1, int x2, int y2, int x3, int y3, int a, int b, int c)
{
    ...
    sla(x1, y1, x3, y3, a, b, c);
    sla(x3, y3, x2, y2, a, b, c);
    return;
...
sm::sm(const sm& a)
{
    ...
    buf_.resize(a.buf_.size());
    memcpy(&buf_[0], &(a.buf_)[0], a.buf_.size());
}

sm& sm::operator=(const sm& a)
{
    w_ = a.w_;
    h_ = a.h_;
